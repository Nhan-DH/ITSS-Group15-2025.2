...
func (r *equipmentRepository) Create(equipment *entity.Equipment) error {
	...
	return r.db.QueryRow(query, equipment.FacilityID, equipment.EquipmentName, equipment.SerialNumber, equipment.Quantity, equipment.Origin, equipment.PurchaseDate, equipment.MaintenanceDeadline, equipment.Status).Scan(&equipment.ID)
}

func (r *equipmentRepository) GetByID(id int) (*entity.Equipment, error) {
	equipment := &entity.Equipment{}
	query := `SELECT id, facility_id, equipment_name, COALESCE(serial_number, ''), COALESCE(quantity, 1), COALESCE(origin, ''), COALESCE(purchase_date, CURRENT_DATE), maintenance_deadline, status FROM "Equipment" WHERE id = $1`
...
func (r *equipmentRepository) GetAll() ([]*entity.Equipment, error) {
	rows, err := r.db.Query(`SELECT id, facility_id, equipment_name, COALESCE(serial_number, ''), COALESCE(quantity, 1), COALESCE(origin, ''), COALESCE(purchase_date, CURRENT_DATE), maintenance_deadline, status FROM "Equipment"`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *equipmentRepository) GetAllPaginated(page, limit int) ([]*entity.Equipment, int, error) {
	...
	// Get paginated data
	query := `SELECT id, facility_id, equipment_name, COALESCE(serial_number, ''), COALESCE(quantity, 1), COALESCE(origin, ''), COALESCE(purchase_date, CURRENT_DATE), maintenance_deadline, status FROM "Equipment" ORDER BY id LIMIT $1 OFFSET $2`
...
func (r *equipmentRepository) Update(equipment *entity.Equipment) error {
	...
	_, err := r.db.Exec(query, equipment.FacilityID, equipment.EquipmentName, equipment.SerialNumber, equipment.Quantity, equipment.Origin, equipment.PurchaseDate, equipment.MaintenanceDeadline, equipment.Status, equipment.ID)
	return err
}
