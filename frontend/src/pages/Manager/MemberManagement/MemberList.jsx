  ...
  return (
    <div className="space-y-6 relative">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Tra cứu Hội viên (Manager)</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Tìm kiếm thông tin để check-in hoặc kiểm tra thời hạn gói tập.
          </p>
        </div>
      </div>
                ...
                members?.map((member) => (
                  <TableRow key={member.id || member.memberCode}>
                    ...
                    <TableCell className="text-right pr-4">
                      {/* Lượt bỏ tính năng xóa cho Manager */}
                      <div className="flex items-center justify-end gap-1">
                        <Link to={`/manager/members/${member.id || member.memberCode}`}>
                          <Button variant="ghost" size="icon" title="Xem chi tiết" className="h-8 w-8">
                            <Eye className="h-4 w-4" />
                          </Button>
                        </Link>
                        <Link to={`/manager/members/${member.id || member.memberCode}/edit`}>
                          <Button variant="ghost" size="icon" title="Chỉnh sửa cập nhật" className="h-8 w-8 text-blue-500">
